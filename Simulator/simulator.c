...
void run()
{
    ...
    while (PC < last_code_line && !should_exit) {
        ...
        if (code[PC][2] == '1' || code[PC][3] == '1' || code[PC][4] == '1') { // uses immidiate
            ...
            if(io_registers[TIMERENABLE] == 1) increase_timer();//change /*cmd with const takes extra cycle*/
            disk_cycles++;
            cycles++;
            cmd_counter++;
        ...
        if(io_registers[TIMERENABLE] == 1) increase_timer(); //change
        cycles++;
        disk_cycles++;
